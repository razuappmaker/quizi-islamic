...
class PrayerTimeService {
  ...
  String formatTimeToBanglaWithPeriod(String time) {
    try {
      ...
      String period = _getBanglaTimePeriod(hour, minute);
      String twelveHourTime = _convertTo12HourFormatWithoutAMPM(hour, minute);
      String banglaTime = _convertToBanglaNumbers(twelveHourTime);

      return '$period $banglaTime';
    } catch (e) {
      return time;
    }
  }

  String _convertTo12HourFormatWithoutAMPM(int hour, int minute) {
    int twelveHour = hour % 12;
    if (twelveHour == 0) twelveHour = 12;

    return '${twelveHour.toString().padLeft(2, '0')}:${minute.toString().padLeft(2, '0')}';
  }

  String _getBanglaTimePeriod(int hour, int minute) {
    int totalMinutes = hour * 60 + minute;

    if (totalMinutes >= 3 * 60 && totalMinutes < 5 * 60 + 30) {
      return 'ভোর';
    } else if (totalMinutes >= 6 * 60 && totalMinutes < 11 * 60) {
      return 'সকাল';
  ...
  String formatNumberToLocal(String text, BuildContext context) {
    ...
    if (!languageProvider.isEnglish) {
      ...
      return _convertToBanglaNumbers(text);
    }
    return text;
  }
